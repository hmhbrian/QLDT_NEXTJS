...
export default function UsersPage() {
  ...
  const validateForm = () => {
    ...
    } else if (!/^[a-zA-Z0-9._%+-]+@(becamex\.com|becamex\.com\.vn)$/.test(newUser.email)) {
      newErrors.email = "Email must be from becamex.com or becamex.com.vn domain.";
    ...
    if (!newUser.password) {
      ...
      newErrors.password = "The password must be at least 6 and at max 100 characters long.";
    }
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-sm"><strong>Mã nhân viên:</strong> {selectedUser.employeeId}</p>
                      <p className="text-sm"><strong>Email:</strong> {selectedUser.email}</p>
                      <p className="text-sm"><strong>Số điện thoại:</strong> {selectedUser.phoneNumber}</p>
                    </div>
                  </div>
                  <div>
                    <h4 className="font-medium mb-2">Thông tin công việc</h4>
                    <div className="space-y-1">
                      <p className="text-sm"><strong>Phòng ban:</strong> {selectedUser.department}</p>
                      <p className="text-sm"><strong>Chức vụ:</strong> {selectedUser.position}</p>
                      <p className="text-sm"><strong>Cấp bậc:</strong> {selectedUser.level?.replace('_', ' ')}</p>
                      <p className="text-sm"><strong>Quản lý:</strong> {selectedUser.manager}</p>
                    </div>
                  </div>
                </div>
              </TabsContent>
      ...
      <Dialog open={isAddingUser} onOpenChange={setIsAddingUser}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Điền thông tin để tạo tài khoản người dùng mới. Các trường có dấu * là bắt buộc.
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            ...
            <div className="grid gap-2">
              ...
              <Input
                ...
                onChange={(e) => setNewUser({ ...newUser, startWork: new Date(e.target.value) })}
              />
            </div>
