...
import { getAllChildDepartments, validateDepartmentTree } from '@/lib/utils/department-tree';
...
export default function DepartmentsPage() {
  ...
  const totalPages = Math.ceil(departments.filter(dept =>
    dept.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
    dept.code.toLowerCase().includes(searchTerm.toLowerCase())
  ).length / pageSize);
  ...
  const handleFixDepartmentStructure = () => {
    if (!validation.valid && validation.issues) {
      ...
      validation.issues.forEach(issue => {
        const dept = updatedDepartments.find(d => d.id === issue.departmentId);
        if (!dept) return;
        ...
        else if (issue.type === 'invalid_level' || issue.type === 'invalid_path') {
          if (dept.parentId) {
            // Cập nhật lại level và path dựa trên phòng ban cha
            const parent = updatedDepartments.find(d => d.id === dept.parentId);
            if (parent) {
  ...
  const handleSaveDepartment = () => {
    try {
      ...
      if (editingDepartment) {
        ...
        const childDepartments = getAllChildDepartments(editingDepartment.id, departments);
          ...
          const updatedDepts = prevDepts.map(dept => {
            ...
            if (childDepartments.some(child => child.id === dept.id)) {
              ...
              const parentDept = dept.parentId ? 
                prevDepts.find(p => p.id === dept.parentId) : null;
              
              if (parentDept) {
                // Get updated parent department (might have been updated above)
                const updatedParent = parentDept.id === editingDepartment.id ?
                  updatedDepartment : parentDept;
  ...
  const handleDeleteDepartmentSubmit = () => {
    ...
    const hasChildren = departments.some(dept => dept.parentId === deletingDepartment?.id);
    
    if (hasChildren) {
      toast({ 
        title: "Không thể xóa", 
        description: "Phòng ban này có các phòng ban con. Vui lòng xóa các phòng ban con trước.", 
        variant: "destructive" 
      });
      return;
    }
        ...
        <TabsContent value="tree" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Quản lý cấu trúc phòng ban phân cấp và mối quan hệ cha-con. Kéo-thả để sắp xếp lại.
              </CardDescription>
            </CardHeader>
            <CardContent>
                  ...
                  {selectedDepartment ? (
                    <div className="space-y-4">
                      <div className="flex justify-between items-start">
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground">Mã: {selectedDepartment.code}</p>
                        </div>
                      ...
                      <div className="grid gap-2">
                        ...
                        <p className="text-sm">{selectedDepartment.description || 'Không có mô tả'}</p>
                      </div>

                      <div className="grid gap-2">
                        <p className="text-sm font-medium">Quản lý</p>
                        <p className="text-sm">{selectedDepartment.managerId || 'Chưa có quản lý'}</p>
                      </div>

                      <div className="grid gap-2">
                        <p className="text-sm font-medium">Trạng thái</p>
                        <Badge variant={selectedDepartment.status === 'active' ? 'default' : 'secondary'}>
                          {selectedDepartment.status === 'active' ? 'Đang hoạt động' : 'Không hoạt động'}
                        </Badge>
                      </div>
                      ...
                      <div className="grid gap-2">
                        ...
                        <p className="text-sm">
                          {selectedDepartment.parentId 
                            ? departments.find(d => d.id === selectedDepartment.parentId)?.name || 'Không tìm thấy'
                      ...
                      <div className="grid gap-2">
                        ...
                        <div>
                          {departments.filter(d => d.parentId === selectedDepartment.id).map(child => (
                            <Badge key={child.id} variant="outline" className="mr-2 mb-2">
                              {child.name}
                            </Badge>
                          ))}
                      ...
                      <div className="grid gap-2">
                        ...
                        <p className="text-sm">{selectedDepartment.path.join(' > ')}</p>
                      </div>
                      ...
                      <div className="pt-2">
                        ...
                        >
                          <PlusCircle className="h-4 w-4 mr-2" /> Thêm phòng ban con
                        </Button>
                      </div>
                    </div>
                  ) : (
                    <div className="flex items-center justify-center h-full border rounded-md border-dashed p-6">
                      <div className="text-center">
                        ...
                        <h3 className="mt-2 text-sm font-semibold">Chưa chọn phòng ban</h3>
                        <p className="mt-1 text-sm text-muted-foreground">
                          Chọn một phòng ban từ cấu trúc cây để xem thông tin chi tiết
                        </p>
                      </div>
                    </div>
                  )}
                ...
                <TableBody>
                  {paginatedDepartments.map((dept) => (
                    <TableRow key={dept.id}>
                      ...
                      <TableCell>
                        {dept.parentId ? 
                          departments.find(d => d.id === dept.parentId)?.name || 'N/A' : 
                          'Không có'}
                      </TableCell>
