import type { TraineeLevel, CourseCategory } from "@/lib/types/course.types";
import type { WorkStatus } from "@/lib/types/user.types";
...
export const getStatusText = (status?: WorkStatus | string) => {
  ...
  switch (status) {
    ...
    case "terminated":
      return "Đã sa thải";
    default:
      return typeof status === "string" ? status : "Không xác định";
  }
};
