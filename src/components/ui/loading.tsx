...
export function Loading({
  ...
  const spinnerClasses = cn(
    "animate-spin-smooth text-primary drop-shadow-sm transition-all duration-300",
    sizeMap[size],
    className
  );
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/90 backdrop-blur-md">
        <div className="flex items-center justify-center rounded-xl bg-card/95 p-8 shadow-2xl border border-border/50">
          <Loader2
            className={cn("animate-spin-smooth text-primary", sizeMap.xl)}
          />
        </div>
      </div>
    );
...
export function Spinner({
  ...
  return (
    <Loader2
      className={cn(
        "animate-spin-smooth text-primary transition-all",
        sizeMap[size],
        className
      )}
    />
