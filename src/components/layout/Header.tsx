...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-10 flex h-16 items-center justify-between border-b bg-background/80 px-4 backdrop-blur-md md:px-6">
      ...
      <div className="flex items-center gap-3"> {/* Added a div to group ThemeToggle and User Dropdown */}
          ...
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" className="flex items-center gap-2 h-10 rounded-full p-1 pr-2 md:pr-3">
              <Avatar className="h-8 w-8">
                <AvatarImage src={user.urlAvatar} alt={user.fullName || user.email} data-ai-hint="user avatar" />
                <AvatarFallback>{getInitials(user.fullName)}</AvatarFallback>
              </Avatar>
              <span className="hidden md:inline text-sm font-medium truncate max-w-[150px]">{user.fullName || user.email}</span>
