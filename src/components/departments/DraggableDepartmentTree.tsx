...
import { buildDepartmentTree, validateDepartmentTree } from '@/lib/utils/department-tree';
...
export function DraggableDepartmentTree({
  ...
  const flatItems = useMemo(() => {
    const flattenedTreeWithLevels = (
      ...
      level = 0,
      result: { id: string; level: number; item: DepartmentInfo & { children?: DepartmentInfo[] } }[] = []
    ) => {
      items.forEach(item => {
        result.push({
          id: item.id,
  ...
  const handleDrop = useCallback((e: React.DragEvent, targetId: string) => {
    ...
    if (sourceDept && targetDept) {
      ...
      const updatedDepartments = departments.map(dept => {
        ...
        if (dept.parentId === draggedId) {
          ...
          if (draggedDept) {
            ...
            return {
              ...dept,
              path: [...targetDept.path, draggedDept.name, ...dept.path.slice(draggedDept.path.length)],
      ...
      if (newValidation.valid) {
        ...
        console.error("Thao tác không hợp lệ sẽ tạo ra vấn đề:", newValidation.issues);
      }
    }
      ...
      const updatedDepartments = departments.map(dept => {
        ...
        if (dept.parentId === draggedId) {
          ...
          if (draggedDept) {
            ...
            return {
              ...dept,
              path: [draggedDept.name, ...dept.path.slice(draggedDept.path.length)],
              level: 1 + pathDiff
            };
          }
        }
      ...
      <TooltipProvider>
        {flatItems.map(({ id, level, item }) => {
          ...
          if (level > 0) {
            ...
            const allParentsExpanded = parentIds.every(pid => expandedNodes.has(pid));
            ...
            <Tooltip key={id} delayDuration={300}>
              <TooltipTrigger asChild>
                <div 
                  className={cn(
                    ...
                    "hover:bg-muted",
                    "draggable-item",
                    dropTarget === id && "bg-accent/50 outline outline-primary/30",
              ...
              <TooltipContent side="right" align="start">
                <div className="space-y-1">
                  ...
                  {item.description && (
                    <p className="text-xs max-w-60 truncate">{item.description}</p>
                  )}
                </div>
              </TooltipContent>
            </Tooltip>
