...
export function StarRatingDisplay({
  ...
  return (
    <div className={cn("flex items-center gap-0.5", className)}> {/* Điều chỉnh khoảng cách cho các ngôi sao có thể sít hơn */}
      {[...Array(fullStars)].map((_, i) => (
        <Star key={`full-${i}`} className={cn(`h-${size} w-${size} text-yellow-400 fill-yellow-400`)} />
      ))}
      {halfStar && <StarHalf className={cn(`h-${size} w-${size} text-yellow-400 fill-yellow-400`)} />}
      {/* Đảm bảo emptyStars không âm */}
      {[...Array(emptyStars < 0 ? 0 : emptyStars)].map((_, i) => (
        <Star key={`empty-${i}`} className={cn(`h-${size} w-${size} text-gray-300 dark:text-gray-500`)} />
      ))}
    </div>
  );
}
